...
// GET /promotions/filter - Return filtered promotions
app.get('/promotions/filter', (req, res) => {
  const { card_type, category, company_name, active_on } = req.query;

  let filtered = promotions;

  if (card_type) {
    filtered = filtered.filter(p => p.card_type.toLowerCase() === card_type.toLowerCase());
  }
  ...
  if (company_name) {
    filtered = filtered.filter(p => p.company_name.toLowerCase().includes(company_name.toLowerCase()));
  }

  if (active_on) {
    const date = new Date(active_on);
    filtered = filtered.filter(p => {
      const start = new Date(p.validity.start_date);
      const end = new Date(p.validity.end_date);
